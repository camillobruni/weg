...
export const Parsers = (() => {
  ...
  function computeStats(pts: TrackPoint[]): TrackStats {
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      if (p.power != null) { powerSum += p.power; powerN++; if (p.power > stats.maxPower) stats.maxPower = p.power; }
      if (p.hr    != null) { hrSum    += p.hr;    hrN++;    if (p.hr    > stats.maxHR)    stats.maxHR    = p.hr; }
      if (p.cad   != null) { cadSum   += p.cad;   cadN++;   }
      if (p.speed != null) { speedSum += p.speed; speedN++; if (p.speed > stats.maxSpeed) stats.maxSpeed = p.speed; }
    ...
    return new Promise((resolve, reject) => {
      ...
      fit.parse(buf, (err: any, data: any) => {
        ...
        try {
          ...
          if (!sessions?.length) { reject(new Error('No sessions in FIT file')); return; }
          ...
          if (!allRecords.length) { reject(new Error('No records in FIT file')); return; }
          ...
          const points: (TrackPoint | null)[] = allRecords.map(r => {
            ...
            const time = r.timestamp ? (r.timestamp instanceof Date ? r.timestamp.getTime() : new Date(r.timestamp).getTime()) : null;
  ...
  async function parseFile(file: File): Promise<any> {
    ...
    switch (ext) {
      ...
      case 'tcx': return parseTCX(buf);
      case 'fit': return parseFIT(buf);
      case 'kml': return parseKML(buf);
